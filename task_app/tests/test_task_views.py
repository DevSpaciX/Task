...
class DataFromFixtureListTests(TestCase):
    def test_get_fixture_data(self):
        ...
        self.assertNotEqual(list(response.context["tasks"]), [])
        self.assertNotEqual(len(list(response.context["tasks"])), 3)


class OwnTaskData(TestCase):
    def test_get_own_data(self):
        self.task = Task.objects.create(
            title="test", is_done=False, created_at=datetime.now()
        )
...
class TaskTagsCheck(TestCase):
    ...
    def test_get_tags(self):
        ...
        response = self.client.get(TASK_URL)
        self.assertEqual(
            list(response.context["tasks"].filter(title="test").first().tags.all()),
